...
export function run(input) {
  ...
  // 1. Get payment rules metafield
  const rulesMetafield = input.paymentCustomization?.metafield?.value;
  console.log("payment-customization: rulesMetafield =", rulesMetafield);
  console.log("payment-customization: cart =", JSON.stringify(input.cart));
  if (!rulesMetafield) {
    return { operations: [] };
  }
  ...
  try {
    ...
    console.error("Failed to parse payment rules metafield JSON:", e);
    return { operations: [] };
  }
  ...
  console.log("payment-customization: paymentMethods available in checkout =", JSON.stringify(paymentMethods));

  for (const rule of rules) {
    if (rule.status !== "active") continue;

    // Evaluate the condition against the cart input
    const isTriggered = evaluateRule(rule, input);
    if (isTriggered) {
      // Target method name to match
      const targetMethod = (rule.error_target || "").toLowerCase().trim();
      const action = rule.delivery_action || "hide"; // For payment, we can reuse delivery_action or treat it as payment_action

      for (const method of paymentMethods) {
        const methodName = (method.name || "").toLowerCase();
        
        // Match by title inclusion or backward check (e.g. "(for testing) bogus gateway" matches "bogus gateway" or "bogus" or "credit card" under Bogus provider)
        const isMatch = methodName.includes(targetMethod) || 
                        targetMethod.includes(methodName) || 
                        (targetMethod.includes("bogus") && methodName.includes("bogus")) ||
                        (targetMethod.includes("bogus") && methodName.includes("credit card"));

        if (isMatch && targetMethod.length > 0) {
          if (action === "hide") {
            operations.push({
              hide: {
          ...
          } else if (action === "rename" && rule.error_message) {
            operations.push({
              rename: {
                paymentMethodId: method.id,
                name: rule.error_message,
              },
            });
          }
        }
  ...
  console.log("payment-customization: final operations =", JSON.stringify(operations));
  return {
    operations,
  };
}
...
function evaluateRule(rule, cartInput) {
  ...
  const results = conditions.map(cond => {
    try {
      const res = evaluateCondition(cond, cartInput);
      console.log(`payment-customization: evaluated condition type=${cond.type} result=${res}`);
      return res;
    } catch (e) {
      console.error(`payment-customization: error evaluating condition type=${cond.type}:`, e);
      return false;
    }
  });
