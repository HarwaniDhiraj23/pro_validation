...
export function run(input) {
  ...
  // 1. Get delivery rules metafield
  const rulesMetafield = input.deliveryCustomization?.metafield?.value;
  console.log("delivery-customization: rulesMetafield =", rulesMetafield);
  console.log("delivery-customization: cart =", JSON.stringify(input.cart));
  if (!rulesMetafield) {
    return { operations: [] };
  }
  ...
  try {
    ...
    console.error("Failed to parse delivery rules metafield JSON:", e);
    return { operations: [] };
  }
  ...
  // Iterate over delivery groups in the cart
  ...
  for (const group of deliveryGroups) {
    ...
    for (const rule of rules) {
      ...
      if (isTriggered) {
        ...
        const action = rule.delivery_action || "hide";

        for (const option of deliveryOptions) {
          const optionTitle = (option.title || "").toLowerCase();
          
          // Match by title inclusion (e.g. "express" matches "Express Shipping")
          if (optionTitle.includes(targetMethod) && targetMethod.length > 0) {
            if (action === "hide") {
              operations.push({
                hide: {
  ...
  console.log("delivery-customization: final operations =", JSON.stringify(operations));
  return {
    operations,
  };
}
...
function evaluateRule(rule, cartInput) {
  ...
  const results = conditions.map(cond => {
    try {
      ...
      console.log(`delivery-customization: evaluated condition type=${cond.type} result=${res}`);
      return res;
    } catch (e) {
      console.error(`delivery-customization: error evaluating condition type=${cond.type}:`, e);
      return false;
    }
  });
